...
var staticDataTable = function staticDataTable() {
    ...
    var bindElement = function bindElement() {
        ...
        Object(__WEBPACK_IMPORTED_MODULE_0__utility__["b" /* addEventValidate */])(validateInput.create);
    };

    var addStaticDashboard = function addStaticDashboard() {
        if (Object(__WEBPACK_IMPORTED_MODULE_0__utility__["d" /* checkError */])(validateInput.create)) {
            return;
        }
        __WEBPACK_IMPORTED_MODULE_0__utility__["a" /* LOADING */].set($("#btn-save-add-static"));
        var static_name = $("#static-name").val();
        var is_public = 0;
        if (!$("#add_statusPrivate").prop("checked")) {
            is_public = 1;
        }
        var desc = $("#desc").val();
        $.ajax({
            ...
            success: function success(res, textStatus, xhr) {
                ...
                __WEBPACK_IMPORTED_MODULE_0__utility__["a" /* LOADING */].reset($("#btn-save-add-static"));
    ...
    var updateDatatableData = function updateDatatableData() {
        var Datatable = [];
        _staticDataTable.fnClearTable();
        $.each(staticList, function (index, item) {
            var title_share = "Share";
            var ret = [];
            ret[0] = item.name;
            ret[1] = item.fname + ' ' + item.lname;
            ret[2] = item.description;
            ret[3] = item.is_public ? "<center><h4><span class=\"badge badge-pill badge-success\">Public</span></h4></center>" : "<center><h4><span class=\"badge badge-pill badge-primary\">Private</span></h4></center>";
        ...
        if (Datatable.length > 0) _staticDataTable.fnAddData(Datatable);
    ...
    var onBtnLookupClick = function onBtnLookupClick(key) {
        window.location.href = END_POINT_WED + "/Dashboards/" + key + "/Edit/Me";
    };
    ...
    var onBtnEditClick = function onBtnEditClick(key) {
        if (modalEdit === null) {
            ...
            Object(__WEBPACK_IMPORTED_MODULE_0__utility__["b" /* addEventValidate */])(validateInput.edit);
        }

        var statusShare = staticList[key].is_public;

        if (statusShare) {
            $("#edit_statusPublic").prop("checked", true);
        } else {
            $("#edit_statusPublic").prop("checked", false);
        }
        ...
        $("#edit_desc").val(staticList[key].description);
        $("#editStatic").modal('show');
    };

    var onBtnSubmitEditClick = function onBtnSubmitEditClick(key) {
        if (Object(__WEBPACK_IMPORTED_MODULE_0__utility__["d" /* checkError */])(validateInput.edit)) {
            return;
        }
        __WEBPACK_IMPORTED_MODULE_0__utility__["a" /* LOADING */].set($("#btn-edit-submit"));
        var is_public = 0;
        if (!$("#edit_statusPrivate").prop("checked")) {
            is_public = 1;
        }
        $.ajax({
    ...
    var onBtnSubmitDeleteClick = function onBtnSubmitDeleteClick(key) {
        __WEBPACK_IMPORTED_MODULE_0__utility__["a" /* LOADING */].set($("#btn-submit-delete"));
        $.ajax({
            ...
            success: function success(res, textStatus, xhr) {
                Object(__WEBPACK_IMPORTED_MODULE_0__utility__["c" /* checkAuthRes */])(xhr);
                __WEBPACK_IMPORTED_MODULE_0__utility__["a" /* LOADING */].reset($("#btn-submit-delete"));
    ...
    var initialDatatable = function initialDatatable() {
        if (_staticDataTable !== null) {
            return false;
        }
        _staticDataTable = $('#example').dataTable({});
    };

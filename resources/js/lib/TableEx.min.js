...
export default class TableEx {
    constructor(el, config) {
        ...
        this.addRow = () => {
            ...
            config.row.amount += 1;
            return el.find('tbody').append(`<tr>${rows}</tr>`);
        }
        ...
        this.setValue = (widget) => {
            ...
            el.find(".col-label").each(function () {
                $(this).val(widget.table.col_labels[i])
                i++;
            });
            i = 0;

            el.find(".row-label").each(function () {
                $(this).val(widget.table.rows[i].label);
                $(this).parent().parent().find('.value-datasource').each(function () {
                   $(this).val(widget.table.rows[i].data[j].val);
                   j++;
                });
                i++;
                j = 0;

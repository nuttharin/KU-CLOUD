  ...
  constructor() {
    ...
    this.createEditdata = (id, myChart, full_id, type) => {
      ...
      $("#edit_chart_data_" + id).click(function () {
        ...
        $("#save_chart_data_" + id).click(function () {
          ...
          if(stateMode == "classic")
          {
            ...
            ctxAttr.attr("maiden-check", "false");
            ...
            ctxAttr.attr('select-agg-list', selectAggList);
            ctxAttr.attr('select-keydata-list', selectDataList);
            ctxAttr.attr('select-datasource-list', selectDatasourceList);

            ctxAttr.removeAttr("select-datasource-list-series");
            ctxAttr.removeAttr("select-keydata-list-series");
          }
          else if(stateMode == "series")
          {
            ...
            ctxAttr.attr('select-datasource-list-series', selectDatasourceListSeries);
            ctxAttr.attr('select-keydata-list-series', selectDataListSeries);

            ctxAttr.removeAttr('period-type');
            ctxAttr.removeAttr('start-value');
            ctxAttr.removeAttr('end-value');
  
            ctxAttr.removeAttr('select-agg-list');
            ctxAttr.removeAttr('select-keydata-list');
            ctxAttr.removeAttr('select-datasource-list');
          }
        ...
        $("#model_edit_chart_data_" + id).on("hidden.bs.modal", function () {
            ...
            if(ctx.attr("maiden-check") == "wait")
            {
              ctx.attr("maiden-check", "true");
            }
        });
      ...
      function setDefaultData(chart)
      {
        ...
        $.ajax({
          url: END_POINT + 'infographic/getServiceByCompany',
          method: 'GET',
          success: (res) => {
            ...
            listInfoDatasource.iotservice.map((data) => {
              $("optgroup[name='opt_iot']").append(`<option value="${index + 1}">${data.iot_name}</option>`);
              index++;
            });

            listInfoDatasource.webservice.map((data) => {
              $("optgroup[name='opt_web']").append(`<option value="${index + 1}">${data.service_name}</option>`);
              index++;
            });
      ...
      function setDefaultDataSeries(chart)
      {
        ...
        $.ajax({
          ...
          success: (res) => {
            ...
            listInfoDatasource.iotservice.map((data) => {
              $("optgroup[name='opt_iot_series']").append(`<option value="${index + 1}">${data.iot_name}</option>`);
              index++;
            });

            listInfoDatasource.webservice.map((data) => {
              $("optgroup[name='opt_web_series']").append(`<option value="${index + 1}">${data.service_name}</option>`);
              index++;
            });

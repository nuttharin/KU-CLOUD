    ...
    constructor(widget) {
        ...
        this.createModalEdit = (inputHtml, bindElement) => {
            ...
            if (!state.editModalIdSelect.length) {
                ...
                ModalEditWidget = `<div class="modal fade" id="${this.widget.editModalId}">
...
class ModalEditWidgetMutiLine extends ModalEditWidget {
    constructor(widget) {
        ...
        let inputHtml = `<div class="row mb-3" id="div-title">
                            <div class="col-12">
                                <lable>Title</label>
...
class MutiLine extends Widget {
    constructor(widget, gridData = null) {
        ...
        this.updateSocket = (service_id, type) => {
            ...
            //         dataset.data.push(Datasource.getValueRealtime(this.datasource[i]) + getRndInteger(0,6));
            ...
            //         //dataset.data.splice(0, 1);
            //     } else {
            //         dataset.data.push(Datasource.getValueRealtime(this.datasource[i],i));
            //     }
            //     i++;
            // });
        }
...
class Datasources {
    constructor() {
        ...
        this.setEventEl = async (el, widget) => {
            el.unbind().change(async function () {
                ...
                if ($(this).val() != '') {
                    ...
                    console.log(datasource,label,state.isRealTime)
            ...
            el.parent().parent().on('focus', '.value-datasource', function () {
                el.parent().parent().find('.data-list.active-data').hide().removeClass('active-data');
                $(this).next().addClass('active-data');
                $(this).next().show();
            })

            $(`#${widget.editModalId} .modal-body`).unbind().on('keyup', '.value-datasource', function () {
        ...
        let bindElement = async () => {
            ...
            $('#Radar').on('change', '.select-datasource', async function () {
            ...
            $('#Mutiline_value,#Gauges,#TextValue,#text-line,#Radar,#Table').on('keyup', '.value-datasource', function () {
                //updateDataList($(this).val(), $(this));
                let value = $(this).val().toLowerCase();
                let datalist = $(this).next();
                datalist.find(".value-data").filter(function () {
...
$(document).ready(function () {
    socket_dashboards = io(`${WS_URL}dashboards?id=${USER_ID}`,{transports: ['websocket']});

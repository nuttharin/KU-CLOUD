...
class ModalEditWidgetMutiLine extends ModalEditWidget {
    constructor(widget) {
        ...
        let inputHtml = `
                        ...
                        <button class="btn btn-primary btn-sm btn-radius mt-2 btn_edit_value_mutiline" >
        ...
        let createDatasourceInput = () => {
            ...
            this.widget.datasources.map(_data => {
                ...
                if (i > 0) {
                    ...
                    htmlData = htmlData.replace('{{delete}}', '');
                }
...
class ModalEditWidgetMap extends ModalEditWidget {
    constructor(widget) {
        ...
        let datasourceInput = `  <div class="col-3">
    ...
    constructor(widget) {
        ...
        let bindElement = () => {
            ...
            state.editModalIdSelect.find("#btn_add_col").unbind().click(function () {
                tableEx.addCol();
                Datasource.addEventTable(state.editModalIdSelect.find(".select-datasource"), widget, true);
            });
            ...
            state.editModalIdSelect.find("#btn_add_row").unbind().click(function () {
                tableEx.addRow();
                Datasource.addEventTable(state.editModalIdSelect.find(".select-datasource"), widget, false);
            });
                ...
                if (!this.isGroupData) {
                    ...
                    for (let i = 0; i < myChart.data.datasets.length; i++) {
                        ...
                        datasource_daily.map(_data => {
                            ...
                            timeLast = new Date(objectPath.get(_data, 'date'));
                        });
                    }
        ...
        this.updateSocket = (service_id, type) => {
            ...
            apiUpdate.map(_i => {
                ...
                myChart.data.datasets[_i].data.push({
                    x: d,
                    y: Datasource.getValueRealtime(this.datasources[_i]) + getRndInteger(0, 6)
                })
            })
        ...
        this.updateData = () => {
            ...
            myChart.data.datasets.forEach((dataset) => {
                ...
                if (!this.isGroupData) {
                    ...
                    dataset.data.push(Datasource.getValueRealtime(this.datasources[i]) + getRndInteger(0, 6));
        ...
        this.updateData = () => {
            ...
            if (!this.isGroupData) {
                ...
                this.datasources.map(data => {
                    ...
                    let value = objectPath.get(datasource, data.value) + getRndInteger(0, 20) //objectPath.get(Datasource.getDatasources(), data.value);
...
class Datasources {
    constructor() {
        ...
        let updateListDatasource = async () => {
            ...
            await listDatasource.webservices.map(async (data, index) => {
                // html += `<div class="row">
            ...
            await listDatasource.iot.map(async (data, index) => {
                opt += `<option value='${data.iot_service_id}'>${data.iot_name}</option>`;
                //apiCount++;
                //await getApiDatasource(index);
            });
            opt += `</optgroup>`
        ...
        let createDataList2 = async (el, datasource, label, isRealtime = true) => {
            ...
            if (label === "web_services") {
                ...
                el.attr("service_id", datasource.iot_service_id)
                exData = await getFlatObjectV2(state.isRealTime === true ? ex.iot_services.realtime[datasource.iot_service_id] : ex.iot_services.static[datasource.iot_service_id]);
            }
        ...
        let getRawData = async (datasource, label) => {
            //url: '/js/company/test-api2.json',
            let data = await axios.get('/js/company/test-api2.json');


            if (label === "web_services") {
                ex.web_services.realtime[datasource.webservice_id] = data.data;
            } else if (label === "iot_services") {
                ex.iot_services.realtime[datasource.iot_service_id] = data.data;
            }
        }
        ...
        let getStaticData = async (datasource, label) => {
            ...
            if (label === "web_services") {
                ...
                ex.iot_services.static[datasource.iot_service_id] = data.data[0].data;
            }
        }


        this.addEventTable = async (el, widget, isAddCol) => {
            state.editModalIdSelect.find(".data-list").hide();
        ...
        this.setEventEl = async (el, widget, isAdd = false) => {
            ...
            $(`#${widget.editModalId} .modal-body`).unbind().on('click', '.remove-radar-labels', function () {
        ...
        this.getValueRealtime = (datasource) => {
            if (datasource.type === "web_services") {
                return objectPath.get(datasources.web_services.realtime[datasource.id], datasource.val);
            } else if (datasource.type === "iot_services") {
                return objectPath.get(datasources.iot_services.realtime[datasource.id], datasource.val);
            }
        }
        ...
        this.getFirstApi = async (service_id, type) => {
            ...
            if (type === "web_services") {
                ...
                if (!hasService(service_id, type)) {
                    realtime = await axios.get('/js/company/test-api2.json');
                    datasources.iot_services.realtime[service_id] = realtime.data
                }
                firstData = await axios.get('/js/company/test-api-static.json');
        ...
        let createFormBodyInputWidget = (type) => {
            ...
            if (type === WIDGET_TYPE.MUTILINE || type === WIDGET_TYPE.MUTILINE_STATIC) {
                ...
                $("#btn_add_col").unbind().click(function () {
                    tableEx.addCol();
                    Datasource.addEventTable($("#addWidget").find(".select-datasource"), widget, true);
                });
                ...
                $("#btn_add_row").unbind().click(function () {
                    tableEx.addRow();
                    Datasource.addEventTable($("#addWidget").find(".select-datasource"), widget, false);
                });

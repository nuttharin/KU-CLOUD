        ...
        let bindWidgetElement = () => {
            ...
            if (this.type.search('static') !== -1) {
                ...
                $(".start_day").unbind().change(function () {
                    let widget = Dashboard.getWidgetById($(this).attr('item'));
                    widget.endDate = null;
                    widget.start_date = moment(new Date($(this).val())).startOf('day').format('YYYY-MM-DD HH:mm:ss');
                    console.log(widget.start_date)
                });

                $(".end_day").unbind().change(function () {
                    let widget = Dashboard.getWidgetById($(this).attr('item'));
                    widget.startDate = null;
                    widget.end_date = moment(new Date($(this).val())).endOf('day').format('YYYY-MM-DD HH:mm:ss');
                    console.log(widget.end_date)
                });


                $(".start_month").unbind().change(function () {
                    let widget = Dashboard.getWidgetById($(this).attr('item'));
                    widget.start_date = moment(new Date($(this).val())).startOf('month').format('YYYY-MM-DD HH:mm:ss');
                    widget.startDate = null;
                    console.log(widget.start_date)
                });
                ...
                $(".end_month").unbind().change(function () {
                    let widget = Dashboard.getWidgetById($(this).attr('item'));
                    widget.endDate = null;
                    widget.end_date = moment(new Date($(this).val())).endOf('month').format('YYYY-MM-DD HH:mm:ss');
                    console.log(widget.end_date)
                });
...
class MutiLineStatic extends Widget {
    constructor(widget, gridData = null) {
        ...
        this.startDate = moment(yesterday).endOf('day').format('YYYY-MM-DD HH:mm:ss');
        this.endDate = moment(yesterday).startOf('day').format('YYYY-MM-DD HH:mm:ss');
        ...
        let getDataStatic = async (datasource) => {
            ...
            if (list) {
                if (datasource.type == "iot_services") {
                    url = `/js/company/test-aggr-${this.periodType}.json`; //`${API_DW}iotService/getInputAggregationForWeb`
                    param = {
    ...
    constructor() {
        ...
        let getStaticData = async (datasource, label) => {
            ...
            if (label === "web_services") {
                ...
                data = await axios.post(`${API_DW}iotService/getInputAggregationForWeb`, {

        ...
        let bindWidgetElement = () => {
            ...
            if (this.type.search('static') !== -1) {
                ...
                $(".start_day").unbind().change(function () {
                    let widget = Dashboard.getWidgetById($(this).attr('item'));
                    widget.startDate = moment(new Date($(this).val())).startOf('day').format('YYYY-MM-DD HH:mm:ss');
                ...
                $(".end_day").unbind().change(function () {
                    let widget = Dashboard.getWidgetById($(this).attr('item'));
                    widget.endDate = moment(new Date($(this).val())).endOf('day').format('YYYY-MM-DD HH:mm:ss');
                    if (moment(widget.endDate).diff(moment(widget.startDate), 'days') >= 0) {
                        widget.updateStatic();
                    }
...
class MutiLineStatic extends Widget {
    constructor(widget, gridData = null) {
        ...
        let getDataStatic = async (datasource) => {
            let url = "";
            let param = "";
            ...
            if (list) {
                if (datasource.type == "iot_services") {
                    ...
                    param = {
                        ...
                        type: PERIOD_STATIC[this.periodType],
        ...
        this.updateStatic = async () => {
            ...
            for (let i = 0; i < datasources.length; i++) {
                let data = await getDataStatic(datasources[i]);
...
class TableStatic extends Widget {
    constructor(widget, gridData = null) {
        ...
        this.createTable = async () => {
            if (!this.isAllow) return;
            this.static = await getDataStatic(this.datasources[0])
            this.el = $('#' + this.widgetId);
            this.el.find('thead').append('<tr></tr>');
            for (let i = 0; i < this.table.col_labels.length; i++) {
                this.el.find('thead tr').append(`<th>${this.table.col_labels[i]}</th>`);
        ...
        let getDataStatic = async (datasource) => {
            ...
            if (list) {
                if (datasource.type == "iot_services") {
                    ...
                    url = API_DW_DATA.getAggrDataWebService//'/js/company/test-aggr.json';
                    param = {
                        nameDW: list.service_name_DW,
                        type: PERIOD_STATIC[this.periodType],
                        start_date: this.startDate,

...
class Widget {
    constructor(widget) {
        ...
        let formateDataSave = (data) => {
            ...
            } else if (data.type === WIDGET_TYPE.RADER) {
    ...
    constructor(widget) {
        ...
        this.updateData = () => {
            ...
            myChart.data.datasets.forEach((dataset) => {
                ...
                dataset.data.push(Datasource.fatoryGetDatasourceValue(this.datasource[0], this.datasource[0][1]));
                $("#" + value).html(Datasource.fatoryGetDatasourceValue(this.datasource[0], this.datasource[0][1]));
            });
...
class RadarEx {
    constructor() {
        ...
        this.createRadarExample = () => {
            if (radarEx === null) {
                let ctx = document.getElementById("example-radar");
                let myRadarChart = new Chart(ctx, config);
            ...
            try {
                ...
                this.el.find('tbody tr').each(function () {
                    ...
                    $(this).find('td').not(':first').each(function () {
                        let data = Datasource.fatoryGetDatasourceValue(rows[i].data[j], rows[i].data[j][1]);
                        $(this).html(data);
                        j++;
                    });
                    i++;
...
class Datasources {
    constructor() {
        ...
        let groupData = {
            ...
            updateDataGroup: (api, el) => {
                try {
                    ...
                    el.find(".data-list").empty();
                    key.map(_key => {
                        el.find(".data-list").append(`<li class="value-data list-group-item" style="cursor:pointer" value="${_key}">${_key}</li>`);
                    });
                    $(".data-list").scrollTop(0);
                    // if (keyInput.length > 2)
                    //     el.parent().find(".data-list").prepend(`<li class="back-value-data list-group-item" back="${keyInput.slice(0, keyInput.length - 1).join(',')}" style="cursor:pointer">Back</li>`);
                ...
                $("#Mutiline_group_data  .data-list").hover(function () {
                    $(this).closest('.value-group-data').focus();
                }, function () {
                    $(this).prev().blur();
                    $(this).hide();
                });

                $("#Mutiline_group_data .value-data").unbind().click(function () {
                ...
                $("#Mutiline_group_data .back-value-data").unbind().click(function () {
                    let back = $(this).attr('back').split(',');
                    let value = "datasource";
                    for (let i = 1; i < back.length; i++) {
                        value += `['${back[i]}']`;
            ...
            createDataValue: (api, el) => {
                try {
                    ...
                    key.map(_key => {
                        ...
                        el.find(".data-list-value").append(`<li class="value-data list-group-item" style="cursor:pointer" value="${_key}">${_key} : ${data}</li>`);
                    });
                ...
                // $("#Mutiline_group_data  .data-list-value").hover(function () {
                //     $(this).closest('.value-datasource').focus();
                // }, function () {
                //     $(this).prev().blur();
                //     $(this).hide();
            ...
            updateDataValue: (api, el) => {
                console.log(api);
                try {
                    console.log($(".value-group-data").val() + '.0' + api);
                    let key = getKeyObject(objectPath.get(datasources, $(".value-group-data").val() + '.0' + el.val()));
                    let keyInput = api.match(REG);
                    el.parent().find(".data-list-value").empty();

                    key.map(_key => {
                        let data = objectPath.get(datasources, `${datasources}${$(".value-group-data").val()}.0${api}.${_key}`);
                        el.parent().find(".data-list-value").append(`<li class="value-data list-group-item" style="cursor:pointer" value="${_key}">${_key} : ${data} </li>`);
                    });
                    $(".data-list-value").scrollTop(0);
                    if (keyInput.length == 1)
                        el.parent().find(".data-list-value").prepend(`<li class="back-value-data list-group-item" back="" style="cursor:pointer">Back</li>`);
                    else {
                        el.parent().find(".data-list-value").prepend(`<li class="back-value-data list-group-item" back="${keyInput.slice(0, keyInput.length - 1).join('.')}" style="cursor:pointer">Back</li>`);
                    }
                ...
                // $("#Mutiline_group_data  .data-list-value,#Mutiline_group_data  .data-list-label").hover(function () {
                //     $(this).closest('.value-datasource').focus();
                // }, function () {
                //     $(this).prev().blur();
                //     $(this).hide();
        ...
        let bindElement = async () => {
            ...
            $('#Gauges,#TextValue,#text-line').on('focus', '.value-datasource', function () {
                $('#Gauges,#TextValue').find('.data-list.active-data').each(function () {
                    $(this).hide();
            ...
            $("#Mutiline_group_data").on('change', '.select-datasource', function () {
                $("#Mutiline_group_data").find(".value-datasource").val('');
                $("#Mutiline_group_data").find(".data-list").hide();
                if ($(this).val() != '') {
                    let datasource = findDatasource($(this).val());
                    groupData.createGroupData(datasource.name, $("#Mutiline_group_data"));
                } else {
                    $(this).parent().parent().next().find(".data-list").empty();
                }
            });
            ...
            $("#Mutiline_group_data").on('change', '.value-group-data', function () {
                let el = $("#Mutiline_group_data");
            ...
            $('#Mutiline_group_data').on('focus', '.value-group-data', function () {
                $('#Mutiline_group_data').find('.data-list.active-data').each(function () {
                    $(this).hide();
                    $(this).removeClass('active-data');
                });
                $(this).next().addClass('active-data');
            ...
            $("#btn-add-value-Mutiline").unbind().click(function () {
                ...
                let html = $("#Mutiline_value").append(formhtml);
                let apiId = $("#Mutiline_value").find('.select-datasource').val();
                if (apiId != '') {
                    let datasource = findDatasource(apiId);
                    html.find(".data-list").hide();
                    html.find(".data-list").removeClass('active-data');
                    createDataList(datasource.name, html.find(".value-datasource").last());
                } else {
                    $(this).parent().parent().next().find(".data-list").empty();
                }
...
class Dashboard {
    constructor() {
        ...
        let getValueRader = () => {
            ...
            let value_datasource = [];
            let color = [];
            $("#Radar").find(".radar-labels:visible").each(function () {
                labels.push($(this).val());
            });
            $("#Radar_value").find(".form-radar-value:visible").each(function () {
                datasets.push({});
                label.push($(this).find('.label-radar').val());
                $(this).find(".value-datasource:visible").each(function () {
                    value_datasource.push($(this).val().match(REG));
                });
                $(this).find(".radar-color:visible").each(function () {
                    color.push($(this).val());
                });
        ...
        let bindElement = () => {
            ...
            $('#Mutiline_value').on('click', '.remove-value', function () {
            ...
            $("input[name='isGroupData']").change(function () {
                isGroupData = $(this).val();
                if (isGroupData == 1) {
                    $("#Mutiline_value").hide();

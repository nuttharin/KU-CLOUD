    ...
    constructor() {
        ...
        let showValues = () => {
            ...
            if (state.selectedTypeDatasource === "web_services") {
                ...
                param['nameDW'] = datasource.service_name_DW
            } else {
        ...
        let addDataAnalysis = () => {
            ...
            if (state.selectedTypeDatasource == "web_services") {
                param['tableDW_name'] = datasource.service_name_DW;
            } else {
                param['tableDW_name'] = datasource.iot_name_DW;
            }
           ...
           //console.log(moment(param.period['end_date']).diff(moment(param.period['start_date']), 'days'))
            if(moment(param.period['end_date']).diff(moment(param.period['start_date']), 'days') < 0){
                showErrorsForElCustom($("#message_date_error"),'End date less than Start date');
        ...
        let onBtnDownloadClick = (index, typeFile) => {
            let file = "";
            let fileZip = "";
            if (typeFile === "arff") {
                file = dataList[index].path_file;
                if (file)
                    fileZip = file.replace(".arff", ".zip");
            } else if (typeFile === "csv") {
                file = dataList[index].path_file_csv;
                if (file)
                    fileZip = file.replace(".csv", ".zip");
            }
